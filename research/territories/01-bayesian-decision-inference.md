...
| 2 | **Two-group Bayesian proportion comparison** — P(A>B), expected loss, uplift posterior | "is variant B actually better or is this noise?" · "model A got 7/10, model B got 5/10 — is that a real difference?" · "which of these two configs should I ship?" · "did my fix actually reduce the error rate?" · "A/B test with tiny samples" | `--a-succ --a-trials --b-succ --b-trials [--rope 0.02]`. **INLINE** | Chi-square/Fisher, which on 7/10 vs 5/10 returns p ≈ 0.65 and tells the agent nothing actionable; and the agent's eyeball, which reads 70% vs 50% as "B is worse". This returns **P(A>B) = 0.79, expected loss from choosing A = 0.014, P(diff inside ±2pp) = 0.06** — three numbers you can actually decide on | **EASY.** Exact closed form: Cook's finite series for P(X>Y) with X~Beta(a,b), Y~Beta(c,d) — `min(a,c)` lgamma terms, microseconds, machine precision (verified vs 200k-draw Monte Carlo, Appendix A). Expected loss has a matching closed form | Non-randomised assignment (B ran later, on warmer cache, on different inputs) — this is an observational comparison masquerading as an experiment · peeking: the agent stopped as soon as B pulled ahead **and** wants a frequentist guarantee (the Bayesian quantity survives, an error rate does not) · outcomes not binary (truncated continuous) |
...
| 4 | **Bayes action under an explicit loss function** (decision table, expected utility, decision-flip threshold) | "which option should I pick?" · "the downside of being wrong isn't symmetric" · "should I roll back or wait?" · "how confident would I have to be to justify doing X?" · "cost of a false positive is way worse than a false negative here" | `--actions ship,rollback --states broken,fine --payoffs ...` plus state probabilities (from a prior row's posterior, or elicited). **INLINE** | Two habits. (a) **Modal-state planning** — optimising for the single most likely world, which is wrong whenever the tail state is expensive. (b) Maximising expected *value* when the loss is asymmetric. Also emits the killer diagnostic: **"the decision flips at P(broken) = 0.19; you're at 0.31"** — which tells the agent how much the answer is worth arguing about | **EASY.** Arithmetic. Extras (EVPI, regret of the runner-up, decision-flip threshold via bisection) are all elementary | Payoffs on incommensurable scales silently added (dollars + hours + reputation) · utility assumed linear over a range where it plainly isn't (ruin, deadlines, rate limits) · states not exhaustive or not mutually exclusive · the probabilities were invented and no sensitivity band was supplied — then report the flip threshold **only**, and suppress the recommendation |
...
| 6 | **Prior sensitivity analysis / robustness gate** (prior ladder + power-scaling + ε-contamination bounds) | "how much of this conclusion is my prior?" · "would a skeptic get the same answer?" · "am I just seeing what I assumed?" · "is this result driven by the data or by my assumptions?" · "how robust is this to how I set it up" | Runs *on top of* any row above. Needs the model's log-prior and log-likelihood, or existing posterior draws. **No extra user input** | Reporting a posterior with no indication that at n=4 it is 85% prior. This is the doctrine feature: it converts "here is a number" into "here is a number, and it survives / does not survive a skeptical prior". Also the honest answer to the Lindley paradox for row 10 | **MODERATE.** Three implementations, all stdlib. (a) *Prior ladder*: refit under reference / weakly-informative / skeptical-null priors, report whether the **decision** flips. (b) *Power-scaling* (Kallioinen et al.): importance-reweight existing draws by `prior(θ)^(α−1)` for α ∈ {0.5, 2} and measure posterior movement — no refit needed, but must be guarded by Pareto-k̂. (c) *ε-contamination / IDM bounds*: for the binomial, Walley's IDM with s=1 gives posterior-mean bounds `[k/(n+s), (k+s)/(n+s)]` in one line | Pareto k̂ above `min(1 − 1/log10(S), 0.7)` under power-scaling → the reweighting is unreliable, refuse and refit · a prior ladder that is secretly one prior in three costumes (all centred at the same place) · the model is so weakly identified that *every* prior in the ladder flips it — report "unidentified", not a range |
...
| # | Model / method | SITUATION (retrieval phrasings) | Minimum viable inputs | Beats what | Stdlib feasibility | Refuse-to-print failure modes |
|---|---|---|---|---|---|---|
| 8 | **Expected value of sample information (EVSI) / "is one more run worth it?"** (+ EVPI) | "should I gather more data before deciding?" · "how many more runs do I need?" · "is it worth the time to test this further?" · "when do I stop investigating?" · "how much would perfect information be worth here?" | The row-4 decision table + the current posterior + `--cost-per-observation`. **INLINE** | The agent's two default behaviours: decide immediately, or run 100 more tests reflexively. EVSI answers it in the decision's own units: *"one more run buys 0.003 expected utility; it costs 0.05 — stop and decide now"*. Also catches the opposite case: *"you're one observation away from flipping this; run it"* | **EASY→MODERATE, and unusually elegant here.** For a two-action decision with a conjugate posterior, **EVSI for m more binomial observations is an exact finite sum over the m+1 possible outcomes** of the beta-binomial predictive — no simulation at all. Gamma-Poisson is a truncated sum. Continuous cases need small Monte Carlo (10⁴ draws, well under a second) | EVPI computed but no decision actually pending (then the number is meaningless theatre) · the cost of sampling isn't in the same units as the payoffs · the "one more observation" would not be exchangeable with the existing ones (the classic: re-running the same flaky test learns almost nothing, but the model assumes it learns a full observation's worth) |
| 9 | **Hierarchical partial pooling with few groups** (J ≈ 3–30) | "I have 6 services each with a handful of observations — which is worst?" · "one of these groups looks terrible but it has tiny n" · "should I treat these as the same or different?" · "rank these variants when they have different sample sizes" · "is this outlier group real or just noise?" | Per-group `(y_j, se_j)` or `(k_j, n_j)`, J ≥ 3, plus an explicit prior on the between-group SD τ. **DATAFILE** | Both naive alternatives, and it is the highest-margin row in the territory. **No pooling** (each group's own rate) is guaranteed to put the smallest-n group at the extremes of the ranking — the "worst service" is almost always the one with 4 observations. **Complete pooling** erases real differences. Partial pooling shrinks each group toward the grand mean by exactly its own noise-to-signal ratio, and the shrinkage is the answer | **MODERATE — and much easier than it looks.** For the normal case with known `se_j`, θ and μ marginalise analytically, leaving a **one-dimensional grid over τ** (the BDA3 eight-schools recipe): `p(τ|y) ∝ p(τ)·√V_μ·Π_j N(y_j \| μ̂, se_j²+τ²)`. **No MCMC required.** Binomial groups need a 2-D grid over (mean, concentration) of a beta-binomial. Verified: J=5, 7-parameter Metropolis version runs 60k iterations in 0.14 s (Appendix A) | **J < 4 → refuse or force an explicit, defended τ prior.** With three groups τ is essentially unidentified and the entire answer is the prior; this is the single most important guard in the territory ([Röver et al. 2021](https://arxiv.org/abs/2007.08352)) · groups not exchangeable (they differ in a known, modellable way — that's a covariate, not a random effect) · `se_j` supplied as a guess · a group with n=1 and no plausible se |
...
| 11 | **Prior elicitation from stated quantiles** (fit Beta / Normal / LogNormal to two quantiles; prior-sample-size check) | "I believe it's around 20% but could be as high as 40%" · "turn my hunch into a prior" · "how do I encode what I already know?" · "I have a rough sense of the range" · "what prior should I use here?" | `--median 0.2 --q95 0.4 --family beta`. **INLINE** | The two failure modes of an agent picking a prior freehand: (a) writing `Beta(2,8)` because it "looks about right", with no check that it implies the beliefs stated; (b) reaching for `Uniform` as "uninformative", which is strongly informative on any transformed scale and — for the beta-binomial — worth a full pseudo-observation in each direction. The fitted prior also reports its **implied prior sample size** (`a+b`), the sanity check that catches "your prior is worth 60 observations and you have 5" | **EASY→MODERATE.** Two-quantile fits: Normal/LogNormal closed form; Beta/Gamma by 2-D root-find (bisection on one parameter with an inner solve, using `I_x(a,b)` and its inverse). Robust and fast | Stated quantiles internally inconsistent (q05 > median) · no distribution in the family fits the stated quantiles within tolerance → refuse rather than fit the nearest thing · the elicited belief was formed **after** seeing the data (double-counting — the most likely abuse when an LLM is its own expert) · a prior sample size exceeding the real one by >5× without an explicit acknowledgement |
